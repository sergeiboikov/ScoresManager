...
-- Format JSON: N'{"Bonuses":{"Value":"Read"},{"Value":"Aliases"},"SubTaskDescription":"Create T-SQL script1","SubTaskName":"Subtask.03.06","SubTaskTopicId":12,"SubTaskMaxScore":12.5,"TaskId":1}' --    Example. EXEC dbo.usp_Subtask_Insert jsn = N'{"Bonuses":{"Value":"Read"},{"Value":"Aliases"},"SubTaskDescription":"Create T-SQL script1","SubTaskName":"Subtask.03.06","SubTaskTopicId":12,"SubTaskMaxScore":12.5,"TaskId":1}' -- =============================================

CREATE OR REPLACE PROCEDURE dbo.usp_SubTask_Insert
(
     jsn            TEXT
)
LANGUAGE plpgsql
...
BEGIN
    ...
    BEGIN
        ...
        --TODO: FILL
                ...
                DROP TABLE IF EXISTS #TEMP_SUBTASK_RESULT;                CREATE TABLE #TEMP_SUBTASK_RESULT (SubTaskId INT, SubTaskName VARCHAR(250), Action VARCHAR(10));                SELECT DISTINCT *
                INTO TEMP_SOURCE
                FROM OPENJSON(jsn)
                WITH (
                         TaskId                INT                '$.TaskId'
                ...
                WHERE tmp.BonusCode IS NOT NULL
                    ...
                    RAISERROR (N'Bonus code: ''%s'' isn''t found', 16, 1, @NoMatchedBonusFromJson);                -- SubTaskBonus
