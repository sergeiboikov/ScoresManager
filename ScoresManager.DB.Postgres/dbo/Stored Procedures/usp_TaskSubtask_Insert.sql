...
*/
/*    Example 1. Add Task and Subtasks: EXEC dbo.usp_TaskSubtask_Insert jsn = N'{"CourseName": "MSBI.DEV.S20", "TaskName": "MSBI.DEV.Task.164", "TaskDescription": "SSIS.Part.5", "TaskTopic": "SSIS.Part.5", 
                                                          "Subtasks":  
                                                                         {"SubTaskName": "Subtask.164.01", "SubTaskDescription": "SSIS project development", "SubTaskTopic": "SSIS", "SubTaskMaxScore": 12.5, "Bonuses":"Readability", "SARG"} 
                                                                        ,{"SubTaskName": "Subtask.164.02", "SubTaskDescription": "SSIS project deployment", "SubTaskTopic": "SSIS", "SubTaskMaxScore": 12.5, "Bonuses":"Readability", "SARG"}            
    ...
    Example 2. Add Task without Subtasks: EXEC dbo.usp_TaskSubtask_Insert jsn = N'{"CourseName": "MSBI.DEV.S20", "TaskName": "MSBI.DEV.Task.164", "TaskDescription": "SSIS.Part.5", "TaskTopic": "SSIS.Part.5"}'
*/-- =============================================

CREATE OR REPLACE PROCEDURE dbo.usp_TaskSubTask_Insert(jsn JSON)
LANGUAGE plpgsql
AS $$
BEGIN

    DECLARE        @NoMatchedBonusFromJson                VARCHAR(250),
                @NoMatchedTaskTopicFromJson            VARCHAR(250),
    ...
    BEGIN
        ...
        --TODO: FILL
                ...
                DROP TABLE IF EXISTS #TEMP_SUBTASK_RESULT;                CREATE TABLE #TEMP_TASK_RESULT (TaskId INT, Action VARCHAR(10));
                CREATE TABLE #TEMP_SUBTASK_RESULT (SubTaskId INT, SubTaskName VARCHAR(250), Action VARCHAR(10));                SELECT *
                INTO TEMP_SOURCE
                FROM OPENJSON(jsn)
                WITH (
                ...
                LEFT JOIN OPENJSON(jsn, N'$.Subtasks')
                WITH (
                ...
                OUTER APPLY OPENJSON(Bonuses)
                    WITH (Bonus VARCHAR(30) '$');                -- Check TaskTopic names
                ...
                WHERE tmp.TaskTopic IS NOT NULL
                    AND tt.TopicId IS NULL                IF (@NoMatchedTaskTopicFromJson IS NOT NULL)
                BEGIN
                    RAISERROR (N'TaskTopic: ''%s'' isn''t found', 16, 1, @NoMatchedTaskTopicFromJson);
                END                -- Check SubTaskTopic names
                ...
                WHERE tmp.SubTaskTopic IS NOT NULL
                    AND stt.TopicId IS NULL                IF (@NoMatchedSubTaskTopicFromJson IS NOT NULL)
                BEGIN
