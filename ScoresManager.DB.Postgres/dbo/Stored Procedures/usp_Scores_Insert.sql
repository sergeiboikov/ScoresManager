...
CREATE OR REPLACE PROCEDURE dbo.usp_Scores_Insert(jsn JSON)
LANGUAGE plpgsql
AS $$
BEGIN

    DECLARE @ReviewerNameFromJson   VARCHAR(100),
            @StudentNameFromJson    VARCHAR(100),
            @CourseNameFromJson     VARCHAR(250),
            @ReviewerID             INT,
            @StudentID              INT,
            @CourseID               INT
    BEGIN
        ...
        --TODO: FILL
                ...
                CREATE TABLE #TEMP_RESULT (SubTaskLogId INT, Action VARCHAR(10));
                ...
                INTO TEMP_SOURCE
                    ...
                    WITH (
                            ...
                            ,ReviewerName   VARCHAR(100)   '$.ReviewerName'
                            ,StudentName    VARCHAR(100)   '$.StudentName'
                            ,TaskName       VARCHAR(250)   '$.TaskName'
                   )
                    CROSS APPLY OPENJSON(jsn, N'$.Subtasks')
                    WITH (
                ...
                FROM TEMP_SOURCE AS t;                SELECT TOP 1 @CourseID = c.CourseId
                ...
                WHERE ut.Name = 'Mentor'
                    ...
                    AND u.Name = @ReviewerNameFromJson;                  IF (@ReviewerID IS NULL)
                BEGIN
                ...
                FROM TEMP_SOURCE AS t;                SELECT TOP 1 @StudentID = u.UserId 
                FROM dbo.User AS u 
                INNER JOIN dbo.CourseStaff sc ON sc.UserId = u.UserId
                INNER JOIN dbo.UserType ut ON ut.UserTypeId = sc.UserTypeId
                WHERE ut.Name = 'Student'
                    AND sc.CourseId = @CourseID
                    AND u.Name = @StudentNameFromJson;                IF (@StudentID IS NULL)
                BEGIN
